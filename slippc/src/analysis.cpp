...
std::string Analysis::statsAsJson() {
  ...
  for(unsigned p = 0; p < 2; ++p) {
    ss << "      " << "{";
    ss << JEND(a) << JUIN(8,"port",                   ap[p].port);
    ss << JEND(a) << JSTR(8,"tag_player",             escape_json(ap[p].tag_player));
    ss << JEND(a) << JSTR(8,"tag_css",                escape_json(ap[p].tag_css));
    ss << JEND(a) << JSTR(8,"tag_code",               escape_json(ap[p].tag_code));
    ss << JEND(a) << JUIN(8,"char_id",                ap[p].char_id);
    ss << JEND(a) << JSTR(8,"char_name",              ap[p].char_name);
    ss << JEND(a) << JUIN(8,"player_type" ,           ap[p].player_type);
    ss << JEND(a) << JUIN(8,"cpu_level" ,             ap[p].cpu_level);
    ss << JEND(a) << JUIN(8,"color"       ,           ap[p].color);
    ss << JEND(a) << JUIN(8,"team_id"     ,           ap[p].team_id);
    ss << JEND(a) << JUIN(8,"start_stocks",           ap[p].start_stocks);
    ss << JEND(a) << JUIN(8,"end_stocks",             ap[p].end_stocks);
    ss << JEND(a) << JUIN(8,"end_pct",                ap[p].end_pct);
    ss << JEND(a) << JUIN(8,"airdodges",              ap[p].airdodges);
    ss << JEND(a) << JUIN(8,"spotdodges",             ap[p].spotdodges);
    ss << JEND(a) << JUIN(8,"rolls",                  ap[p].rolls);
    ss << JEND(a) << JUIN(8,"dashdances",             ap[p].dashdances);
    ss << JEND(a) << JUIN(8,"l_cancels_hit",          ap[p].l_cancels_hit);
    ss << JEND(a) << JUIN(8,"l_cancels_missed",       ap[p].l_cancels_missed);
    ss << JEND(a) << JUIN(8,"techs",                  ap[p].techs);
    ss << JEND(a) << JUIN(8,"walltechs",              ap[p].walltechs);
    ss << JEND(a) << JUIN(8,"walljumps",              ap[p].walljumps);
    ss << JEND(a) << JUIN(8,"walltechjumps",          ap[p].walltechjumps);
    ss << JEND(a) << JUIN(8,"missed_techs",           ap[p].missed_techs);
    ss << JEND(a) << JUIN(8,"ledge_grabs",            ap[p].ledge_grabs);
    ss << JEND(a) << JUIN(8,"air_frames",             ap[p].air_frames);
    ss << JEND(a) << JUIN(8,"wavedashes",             ap[p].wavedashes);
    ss << JEND(a) << JUIN(8,"wavelands",              ap[p].wavelands);
    ss << JEND(a) << JUIN(8,"neutral_wins",           ap[p].neutral_wins);
    ss << JEND(a) << JUIN(8,"pokes",                  ap[p].pokes);
    ss << JEND(a) << JUIN(8,"counters",               ap[p].counters);
    ss << JEND(a) << JUIN(8,"powershields",           ap[p].powershields);
    ss << JEND(a) << JUIN(8,"shield_breaks",          ap[p].shield_breaks);
    ss << JEND(a) << JUIN(8,"grabs",                  ap[p].grabs);
    ss << JEND(a) << JUIN(8,"grab_escapes",           ap[p].grab_escapes);
    ss << JEND(a) << JUIN(8,"taunts",                 ap[p].taunts);
    ss << JEND(a) << JUIN(8,"meteor_cancels",         ap[p].meteor_cancels);
    ss << JEND(a) << JFLT(8,"damage_dealt",           ap[p].damage_dealt);
    ss << JEND(a) << JUIN(8,"hits_blocked",           ap[p].hits_blocked);
    ss << JEND(a) << JUIN(8,"shield_stabs",           ap[p].shield_stabs);
    ss << JEND(a) << JUIN(8,"edge_cancel_aerials",    ap[p].edge_cancel_aerials);
    ss << JEND(a) << JUIN(8,"edge_cancel_specials",   ap[p].edge_cancel_specials);
    ss << JEND(a) << JUIN(8,"teeter_cancel_aerials",  ap[p].teeter_cancel_aerials);
    ss << JEND(a) << JUIN(8,"teeter_cancel_specials", ap[p].teeter_cancel_specials);
    ss << JEND(a) << JUIN(8,"phantom_hits",           ap[p].phantom_hits);
    ss << JEND(a) << JUIN(8,"no_impact_lands",        ap[p].no_impact_lands);
    ss << JEND(a) << JUIN(8,"shield_drops",           ap[p].shield_drops);
    ss << JEND(a) << JUIN(8,"pivots",                 ap[p].pivots);
    ss << JEND(a) << JUIN(8,"reverse_edgeguards",     ap[p].reverse_edgeguards);
    ss << JEND(a) << JUIN(8,"self_destructs",         ap[p].self_destructs);
    ss << JEND(a) << JUIN(8,"stage_spikes",           ap[p].stage_spikes);
    ss << JEND(a) << JUIN(8,"short_hops",             ap[p].short_hops);
    ss << JEND(a) << JUIN(8,"full_hops",              ap[p].full_hops);
    ss << JEND(a) << JUIN(8,"shield_time",            ap[p].shield_time);
    ss << JEND(a) << JFLT(8,"shield_damage",          ap[p].shield_damage);
    ss << JEND(a) << JFLT(8,"shield_lowest",          ap[p].shield_lowest);
    ss << JEND(a) << JUIN(8,"total_openings",         ap[p].total_openings);
    ss << JEND(a) << JFLT(8,"mean_kill_openings",     ap[p].mean_kill_openings);
    ss << JEND(a) << JFLT(8,"mean_kill_percent",      ap[p].mean_kill_percent);
    ss << JEND(a) << JFLT(8,"mean_opening_percent",   ap[p].mean_opening_percent);
    ss << JEND(a) << JUIN(8,"galint_ledgedashes",     ap[p].galint_ledgedashes);
    ss << JEND(a) << JFLT(8,"mean_galint",            ap[p].mean_galint);
    ss << JEND(a) << JUIN(8,"max_galint",             ap[p].max_galint);
    ss << JEND(a) << JUIN(8,"button_count",           ap[p].button_count);
    ss << JEND(a) << JUIN(8,"cstick_count",           ap[p].cstick_count);
    ss << JEND(a) << JUIN(8,"astick_count",           ap[p].astick_count);
    ss << JEND(a) << JFLT(8,"actions_per_min",        ap[p].apm);
    ss << JEND(a) << JUIN(8,"state_changes",          ap[p].state_changes);
    ss << JEND(a) << JFLT(8,"states_per_min",         ap[p].aspm);
    ss << JEND(a) << JUIN(8,"shieldstun_times",       ap[p].shieldstun_times);
    ss << JEND(a) << JUIN(8,"shieldstun_act_frames",  ap[p].shieldstun_act_frames);
    ss << JEND(a) << JUIN(8,"hitstun_times",          ap[p].hitstun_times);
    ss << JEND(a) << JUIN(8,"hitstun_act_frames",     ap[p].hitstun_act_frames);
    ss << JEND(a) << JUIN(8,"wait_times",             ap[p].wait_times);
    ss << JEND(a) << JUIN(8,"wait_act_frames",        ap[p].wait_act_frames);
    ss << JEND(a) << JUIN(8,"used_norm_moves",        ap[p].used_norm_moves);
    ss << JEND(a) << JUIN(8,"used_spec_moves",        ap[p].used_spec_moves);
    ss << JEND(a) << JUIN(8,"used_misc_moves",        ap[p].used_misc_moves);
    ss << JEND(a) << JUIN(8,"used_grabs",             ap[p].used_grabs);
    ss << JEND(a) << JUIN(8,"used_pummels",           ap[p].used_pummels);
    ss << JEND(a) << JUIN(8,"used_throws",            ap[p].used_throws);
    ss << JEND(a) << JUIN(8,"total_moves_used",       ap[p].total_moves_used);
    ss << JEND(a) << JUIN(8,"total_moves_landed",     ap[p].total_moves_landed);
    ss << JEND(a) << JFLT(8,"move_accuracy",          ap[p].move_accuracy);
    ss << JEND(a) << JFLT(8,"actionability",          ap[p].actionability);
    ss << JEND(a) << JFLT(8,"neutral_wins_per_min",   ap[p].neutral_wins_per_min);
    ss << JEND(a) << JFLT(8,"mean_death_percent",     ap[p].mean_death_percent);
